...
#Date: 31AUG2020
#Author: John Pearce
#To install 'SOMmix' functions execute file as 'Source'
...
map.comp.plot<-function(som.obj, col.mod=diverging_hcl, border.style="straight"){
  ...
  par(mfrow=c(round(sqrt(p)),round(sqrt(p))), mar=c(1,4,2,1), family='serif')
...
map.fit.plots<-function(som.obj, lab.cex=1){
  ...
  par(mfrow=c(4,2), mar=c(5,4,2,1),ask=FALSE, family='serif', pty="m")
  ...
  legend("bottomright", legend=c("OBS", "SOM", "K-Means"), pch=c(20,1,3),
         col=c("lightgrey","black","black"), cex=1.5)
...
exp.metric<-function(som.obj){
  ...
  #Coordinates
  som.coords<-som.summ$COORDINATES

  exp.tab<-merge(class.tab, som.coords, by="SOM_ID", all.x=TRUE)
