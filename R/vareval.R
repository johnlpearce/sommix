############################################################################################
#vareval: A Collection of functions to perform generalized data evaluation
############################################################################################
...
#'@param x is a data frame object
...
vareval  <-  function(x, labsize=1, symsize=2, colormod=NULL, varlabs=NULL)
{
  #Set up data
  data<-data.frame(x)
  vars<-names(data)
  ...
  #Evaluation Plots
  opar<-graphics::par()
  graphics::par(mar=c(3,2,1.5,1), xaxs="r")
  ...
  graphics::boxplot(scale(data), col=colormod, las=2, ylab="Standardized Values (Mean=0; SD=1)", names=varlabs)
